...
import {
    ...
    serviceBusTopicTriggerAndOutput,
} from './resources/serviceBus';
...
describe('serviceBus', () => {
    ...
    it('queue trigger and output', async () => {
        ...
        const sender = client.createSender(serviceBusQueueOneTriggerAndOutput);
    ...
    it('topic trigger and output', async () => {
        const message = getRandomTestData();
        const sender = client.createSender(serviceBusTopicTriggerAndOutput);
        const batch = await sender.createMessageBatch();
        batch.tryAddMessage({ body: message });
        await sender.sendMessages(batch);
    ...
    it('trigger and output, cardinality many', async () => {
        ...
        const sender = client.createSender(serviceBusQueueManyTriggerAndOutput);
        const batch = await sender.createMessageBatch();
